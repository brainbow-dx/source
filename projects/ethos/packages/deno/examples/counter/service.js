...
export class CounterService
{
    ...
    async serve()
    {
        try
        {
            Deno.serve({
                ...this.#apiOptions,
                onListen: ({ hostname, port }) => console.log(`API listening on http://${hostname}:${port}`),
            }, this.handleApiRequest.bind(this));
            Deno.serve({
                ...this.#adminOptions,
                onListen: ({ hostname, port }) => console.log(`Admin listening on http://${hostname}:${port}`),
            }, this.handleAdminRequest.bind(this));
        }
        catch (exc)
        {
