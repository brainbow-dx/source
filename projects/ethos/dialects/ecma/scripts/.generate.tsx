...
if (args.reset === true && await exists(OUT_DIR)) {
    console.warn($.brightYellow(`Got reset!`), `Remove ${OUT_DIR}?`);

    // Confirm the destruction of the previous directory ..
    const encoder = new TextEncoder();
    const confirm = encoder.encode($.dim.italic(`Press <enter> to continue; ctrl+c to exit.`));
    await Deno.stdout.write(confirm);

    const stdinBuf = new Uint8Array(100);
    if (await Deno.stdin.read(stdinBuf) && args.verbose) {
        // Re-iterate intent to destroy.
        const decoder = new TextDecoder();
        const _input = decoder.decode(stdinBuf);
        console.log($.magenta.bold(`Roger, go to boom.`));
    }
