...
impl<S: AsRef<Path>> EcmaModule<S> {
    pub fn try_parse(sourcemap: Arc<SourceMap>, entry_module: S) -> Result<Self, TemplateError> {
        let entrypoint_source = sourcemap.load_file(entry_module.as_ref()).map_err(TemplateError::LoadFailed)?;
        let entrypoint_source = SourceFileInput::from(entrypoint_source.as_ref());
        ...
        let lexer = Lexer::new(syntax, es_version, entrypoint_source, Some(&mut comments));
