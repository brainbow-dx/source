    ...
    match sd_version {
        StableDiffusionVersion::XlInpaint | StableDiffusionVersion::V2Inpaint | StableDiffusionVersion::V1_5Inpaint => {
            let inpaint_mask = mask_path
                .ok_or_else(|| anyhow::anyhow!("An inpainting model was requested but mask-path is not provided."))?;
...
fn run(args: Args) -> Result<()> {
    ...
    let sd_config = match sd_version {
        ...
        StableDiffusionVersion::Turbo =>
            stable_diffusion::StableDiffusionConfig::sdxl_turbo(sliced_attention_size, height, width),
    };
    ...
    for idx in 0..num_samples {
        ...
        let latents = match &init_latent_dist {
            ...
            None => {
                let latents =
                    Tensor::randn(0f32, 1f32, (bsize, 4, sd_config.height / 8, sd_config.width / 8), &device)?;
