...
cd "$SCRIPT_DIR" || exit 1 # What code here?

ABY_SDK_DIR="$(realpath '../packages/sdk')"

UNITY_PROJECT_DIR="$(realpath '../runtimes/Unity')"
UNITY_PLUGIN_DIR="$UNITY_PROJECT_DIR/Assets/Plugins/AbySDK"

BUILD_TARGET="${1:-Release}"

LIB_NAME="abysdk.dll"

echo "Building SDK ($BUILD_TARGET; $ABY_SDK_DIR)"
cd "$ABY_SDK_DIR" || exit 1 # TODO: What code here?
...
LIB_BUILD_PATH="$ABY_SDK_DIR/bin/$BUILD_TARGET/netstandard2.1/$LIB_NAME"
...
# if [[ " $* " =~ " --bootstrap " ]];
# then
#     UNITY_TEST_PATH="ABY.Unity.Editor.Aby.Tests.EcmaRuntime.CanMount"
