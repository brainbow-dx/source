...
// omitted: sqlite3_setlk_timeout (not present in libsql's SQLite fork)
unsafe extern "C" {
...
// `sqlite3_set_errmsg` was added in upstream SQLite 3.51.0 ("intended for use by SQLite
// extensions or wrappers" to overwrite a connection's own error state — see its doc comment in
// the real crate's vendored `sqlite3.h`). libsql's fork (based on 3.45.1) doesn't implement it,
// and there's no other public SQLite API this shim could use to replicate its effect (it needs
// to write to `struct sqlite3`'s own private error-state fields, which are opaque outside
// SQLite's own C source) — so this can't be a faithful reimplementation, only a deliberate stand-
// in. `rusqlite::Connection::set_errmsg` is the only caller anywhere, gated behind rusqlite's
// `modern_sqlite` feature (which `bundled` unconditionally pulls in, which is why this has to
// exist at all) — nothing in `deno_kv`/`deno_cache`/`deno_node_sqlite`/`deno_webstorage` (the
// crates in this workspace that pull rusqlite in with that feature) calls it as of this writing.
// Panics instead of returning a plausible-looking error code: if a future dependency bump ever
// starts actually calling this, that should fail loudly and immediately, not silently misbehave.
pub unsafe extern "C" fn sqlite3_set_errmsg(_db: *mut sqlite3, _errcode: ::core::ffi::c_int, _z_err_msg: *const ::core::ffi::c_char) -> ::core::ffi::c_int {
    panic!("sqlite3_set_errmsg has no real implementation in escher's libsqlite3-sys shim (see this function's own doc comment) — something started calling it, which needs a real decision, not this stub");
}
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
// omitted: sqlite3_str_free (not present in libsql's SQLite fork)
unsafe extern "C" {
...
// omitted: sqlite3_str_truncate (not present in libsql's SQLite fork)
unsafe extern "C" {
...
// omitted: sqlite3_db_status64 (not present in libsql's SQLite fork)
unsafe extern "C" {
...
// omitted: sqlite3_carray_bind_v2 (not present in libsql's SQLite fork)
// omitted: sqlite3_carray_bind (not present in libsql's SQLite fork)
