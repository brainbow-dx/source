...
//! supporting SVG data — confirmed live as a real, silent failure: `NSImage::initWithData` on
//! this SVG data returned `None` (not a garbled/blank image, a hard `None`), so `NodeKind::
//! Button`'s spawn code took its "icon name not recognized" fallback path and rendered the
//! button's plain-text label instead — invisible for glyphs like the chevrons (plain characters,
//! already monochrome), glaring for the pin button (a full-color 📌 emoji suddenly appearing in
//! an otherwise all-gray toolbar). `NSImage` decodes PNG data without question, so each `.svg`
//! here has a same-named `.png` twin (`rsvg-convert -w 48 -h 48 <name>.svg -o <name>.png` — 48px
//! is 3x a 16pt toolbar icon, comfortable headroom for Retina) checked in alongside it; the
//! `.svg` stays for provenance/re-rasterizing if an icon ever needs to change, but nothing in
//! this crate reads it directly. Each source `.svg`'s `stroke="currentColor"` was changed to
//! `stroke="black"` before rasterizing (a straightforward derivative edit ISC explicitly
//! permits) — `NodeKind::Button` loads the PNG as a template `NSImage` (`setTemplate(true)`),
//! which macOS re-tints from a plain black-on-transparent source.
//!
