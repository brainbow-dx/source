...
//! interfaces) — the same crate and the same API shape `runtimes/bevy/src/legacy/webview.rs`
...
//! deliver it — typically within a frame or two of `attach` returning, never synchronously
...
//! this same thread — nothing here is genuinely multi-threaded) populates it once the real
...
pub struct WebViewInner {
    ...
    /// of dropping it — applied the moment the completion callback populates `ready` (see
...
impl Drop for WebViewInner {
    ...
    /// process's side of this controller/webview pair) — the real equivalent of the macOS
...
fn subclass_for_resize(hwnd: HWND, top_inset: f64, left_inset: Rc<RefCell<f64>>, controller: Controller) {
    ...
    unsafe extern "system" fn subclass_proc(hwnd: HWND, msg: UINT, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
        ...
        // ..., GWLP_WNDPROC, ...)` call — a valid `WNDPROC`-typed value reinterpreted as
