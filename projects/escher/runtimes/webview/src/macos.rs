...
/// no in-flight state for `webView:stopURLSchemeTask:` to actually cancel, so that method is a
...
pub struct WebViewInner {
    ...
    /// `None` when `attach` wasn't given a `CustomSchemeHandler` — nothing to keep alive.
...
pub fn attach(
    ...
    // `WKWebView.isInspectable` (macOS 13.3+) — not yet in `objc2-web-kit` 0.3.2's bindings, same
