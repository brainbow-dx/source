...
//! `Page::Process`'s scrollback. Output streams in live, line-by-line, for `run_js_command`/
//! `run_deno_command`; it is held until classified for `run_shell_command` (see its own doc
//! comment for why). Split out of `main.rs` to keep that file from growing further as commands
//! multiply. `main.rs` still owns `AppState`'s `spawn_js_command`/`spawn_shell_command`/
//! `spawn_shape_command` (they close over `self`'s fields, so they can't move here), but the
//! actual subprocess mechanics live in this module.
...
use ethos_deno::host_actions::HostActions;
...
/// It goes through `ethos-deno`'s own API (see `run_embedded_js` below), not `deno_core`/
/// `deno_runtime` directly, and is linked into `escher-anvil` instead of shelled out to as a
/// separate binary. Keeping V8 out of this process "to keep the two runtimes decoupled" isn't a
/// real constraint: when a host wants an engine embedded, embed it. Doing so here means JS-backed
/// commands work with nothing but this one binary, no `ethos`/`deno` install required at all.
/// `run_deno_command`/`run_reject_router` below still shell out to the real `deno` CLI. That's for
/// a different, still-real reason (`ethos-deno`'s embedded runtime has no TypeScript-stripping or
/// JSX support at all), not this one, so those stay as they are.
///
...
/// callers get. The *return value*, though, comes back directly from calling the module's exported
...
/// The actual embed happens entirely through `ethos-deno`'s own API (`RuntimeStdio`, and
...
/// It is opaque to this function, script-defined shape, same "one string, script decides what it
