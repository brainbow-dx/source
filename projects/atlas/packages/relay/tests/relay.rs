...
async fn recv_json(socket: &mut (impl StreamExt<Item = Result<Message, tokio_tungstenite::tungstenite::Error>> + Unpin)) -> serde_json::Value {
...
#[tokio::test]
async fn two_peers_discover_each_other_and_relay_a_handshake() {
    ...
    alice.send(Message::Text(r#"{"type":"join","room":"test-room"}"#.into())).await.unwrap();
    let alice_joined = recv_json(&mut alice).await;
    assert_eq!(alice_joined["type"], "joined");
    let alice_id = alice_joined["peer_id"].as_str().expect("alice has a peer_id").to_string();
    assert_eq!(alice_joined["peers"].as_array().unwrap().len(), 0, "alice is first in the room, no peers yet");

    bob.send(Message::Text(r#"{"type":"join","room":"test-room"}"#.into())).await.unwrap();
    let bob_joined = recv_json(&mut bob).await;
    assert_eq!(bob_joined["type"], "joined");
    let bob_id = bob_joined["peer_id"].as_str().expect("bob has a peer_id").to_string();
    assert_eq!(bob_joined["peers"].as_array().unwrap()[0], alice_id, "bob sees alice already in the room");
    ...
    alice.send(Message::Text(offer.into())).await.unwrap();
    ...
    bob.send(Message::Text(answer.into())).await.unwrap();
    ...
    bob.send(Message::Text(candidate.into())).await.unwrap();
