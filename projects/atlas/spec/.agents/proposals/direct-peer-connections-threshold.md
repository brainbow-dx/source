...
**Update, 2026-08-20**: `relay.rs` grew a second data channel (combat events, reliable/ordered, alongside position's existing best-effort one) for Mario's own cross-instance combat sync, which made the copy-pasted-boilerplate problem concrete rather than hypothetical — raised directly ("building out a relay is a lot of boilerplate... I'd like it to be as simple as defining startup and config and then passing handlers"). A real implementation plan for the generic client this update calls for now exists: `generic-relay-client.md` in this same folder — an `atlas_relay::client` module behind a new opt-in `client` Cargo feature (so signaling-server-only consumers don't gain a `webrtc` dependency), a builder taking typed per-channel handlers, and a migration plan that ends with `relay.rs` itself becoming the first real consumer. Not started; doesn't block tonight's LAN playtest, which keeps using the current hand-rolled version.
